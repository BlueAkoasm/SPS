#ifndef LIO_LASER_MAPPING_H
#define LIO_LASER_MAPPING_H
...
class LaserMapping {
   public:
    ...
    void CreatePointCovVector(std::vector<PointWithCov>& pv_vec, CloudPtr scan_);
    void calcBodyCov(common::V3D &pb, const float range_inc, const float degree_inc, common::M3D &cov);
    void CalcWorldCov(PointWithCov& pv, common::M3D rot);
    void calLidarPointContribution(std::vector<bool>& used);
    void calcMutualInformation(common::V3F p_body, common::V3F point_world, common::VV3D& near_point, common::VM3D& near_points_cov, int index, double& result);
    void calcKL(common::V3F p_body, PointVector near_points, common::VM3D& cov_near, double& result);
    void calcMI(common::V3F p_body, common::VM3D& cov_near, double& result);
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr ColoredPointCloud(PointCloudType::Ptr& in,PointCloudType::Ptr& body, std::unordered_set<size_t> highlight_idx);
   private:
    template <typename T>
   ...
   private:
    ...
    double MI_threshold = 0.9;
    double MP_threshold = 2.0;
    bool MI_used = false;
    ...
    std::vector<common::VM3D> cov_nearest;
    std::vector<bool> lowpoints;  // 改用 vector
    std::vector<StatePose> HistoricPoses;
    common::M6D P_lcx = common::M6D::Identity();
    ...
    std::unordered_set<size_t> highlight_index;
    common::VM3D nearest_cov_;
    common::VV3D residualsAvg_;
    common::VM3D residualsCov_;
    common::VM3D point_this_cov;
    common::VV3D cur_cov_;
    common::VV3D nearest_pos_;
    common::VV3D point_this_pos_;
    std::vector<double> s_cov;
    std::vector<double> s_cov_final;
