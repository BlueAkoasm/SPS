...
class IVox {
   public:
    ...
    static bool custom_compare(DistPoint &a, DistPoint &b) {
        return a.cov.trace() < b.cov.trace();  // 
    }
    ...
    void AddPointsWithCov(const std::vector<PointWithCov>& pointswithcov_to_add);
    ...
    // get nn with cov
    bool GetClosestPoint(const PointType& pt, PointVector& closest_pt, common::VM3D& cov, int max_num = 5, double max_range = 5.0);
...
template <int dim, IVoxNodeType node_type, typename PointType>
bool IVox<dim, node_type, PointType>::GetClosestPoint(const PointType& pt, PointVector& closest_pt, common::VM3D& cov, int max_num, double max_range)
{
    std::vector<DistPoint> candidates;
    // std::cout<<"GetClosestPoint"<<std::endl;
    candidates.reserve(max_num * nearby_grids_.size());
...
// #define INNER_TIMER
    ...
    if (candidates.size() <= max_num) {
        ...
        std::nth_element(candidates.begin(), candidates.begin() + max_num - 1, candidates.end());
        // std::sort(candidates.begin(), candidates.end(), custom_compare);
        candidates.resize(max_num);
    }
    std::nth_element(candidates.begin(), candidates.begin(), candidates.end());
    ...
    for (auto& it : candidates) {
        closest_pt.emplace_back(it.Get());
        // LOG(INFO)<<it.Getcov();
        cov.emplace_back(it.Getcov());
    }
    if(cov.size() == 0) LOG(INFO)<<"GETCLOSETPOINT"<<closest_pt.size();
    return closest_pt.empty() == false;    
}
...
template <int dim, IVoxNodeType node_type, typename PointType>
void IVox<dim, node_type, PointType>::AddPointsWithCov(const std::vector<PointWithCov>& pointswithcov_to_add) {
    std::for_each(std::execution::unseq, pointswithcov_to_add.begin(), pointswithcov_to_add.end(), [this](const auto pt) {
        auto key = Pos2Grid(ToEigen<float, dim>(*(pt.point_world)));
        ...
        if (iter == grids_map_.end()) {
            ...
            grids_cache_.front().second.InsertPoint(*(pt.point_world));
            grids_cache_.front().second.InsertPointWithCov(pt);
            ...
            iter->second->second.InsertPoint(*(pt.point_world));
            iter->second->second.InsertPointWithCov(pt);
            grids_cache_.splice(grids_cache_.begin(), grids_cache_, iter->second);
            grids_map_[key] = grids_cache_.begin();
        }
    });
