...
public:
    PointWithCov()
    {
        cov = common::M3D::Identity();
    };
    PointWithCov(PointType* point_, PointType* point_world_): point(point_), point_world(point_world_)
    {
        cov = common::M3D::Identity();
    }
    PointWithCov(PointType* point_, PointType* point_world_, common::M3D cov_): point(point_), point_world(point_world_), cov(cov_){};
    

    PointType* point;
    PointType* point_world = new PointType();
    common::M3D cov;
    common::M3D cov_world;
    common::M6D contribution;
};
typedef struct Plane {
    ...
    common::V3D y_normal;
    common::V3D x_normal;
    common::M3D covariance;
    common::M6D plane_cov;
    float radius = 0;
...
class IVoxNode {
   public:
    ...
    void InsertPointWithCov(const PointWithCov& pt);
   ...
   private:
    ...
    std::vector<PointWithCov> pointswithcov;
...
struct IVoxNode<PointT, dim>::DistPoint {
    ...
    int idx = 0;
    common::M3D cov = common::M3D::Identity();

    DistPoint() = default;
    DistPoint(const double d, IVoxNode* n, const int i) : dist(d), node(n), idx(i) {}
    DistPoint(const double d, IVoxNode* n, const int i, const common::M3D& cov_) : dist(d), node(n), idx(i), cov(cov_) {}
    PointT Get() { return node->GetPoint(idx); }
    common::M3D Getcov(){return cov;};

    inline bool operator()(const DistPoint& p1, const DistPoint& p2) { return p1.dist < p2.dist; }
    // inline bool operator()(const DistPoint& p1, const DistPoint& p2) { return p1.cov.determinant() < p2.cov.determinant(); }

    inline bool operator<(const DistPoint& rhs) { return dist < rhs.dist; }
    // inline bool operator<(const DistPoint& rhs) { return cov.determinant() < rhs.cov.determinant(); }
};
...
template <typename PointT, int dim>
void IVoxNode<PointT, dim>::InsertPointWithCov(const PointWithCov& pt) {
    pointswithcov.emplace_back(pt);
}
...
// #define INNER_TIMER
    ...
    // for (int kk=0; kk<points_.size(); kk++) {
    //     PointT& pt = points_[kk];
    //     PointWithCov pv = pointswithcov[kk];
#ifdef INNER_TIMER
