#ifndef LIO_IMU_PROCESSING_H
#define LIO_IMU_PROCESSING_H
...
class ImuProcess {
   public:
    ...
    std::vector<IMURelativeDelta> IMURelativePose_;
...
void ImuProcess::UndistortPcl(const common::MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state,
    ...
    IMURelativeDelta IMURel;
    if (!IMUpose_.empty()) {
        // 获取起始和结束时刻的IMU状态
        const auto& imu_start = IMUpose_.front();  // T_{WB_{k-1}} (上一帧)
        const auto& imu_end = IMUpose_.back();     // T_{WB_k}     (当前帧)
    
        // 提取旋转矩阵 R_{WB_{k-1}} 和 R_{WB_k}
        const Eigen::Matrix3d R_start = common::MatFromArray(imu_start.rot);
        const Eigen::Matrix3d R_end = common::MatFromArray(imu_end.rot);
    
        // 提取平移向量 t_{WB_{k-1}} 和 t_{WB_k}
        const Eigen::Vector3d t_start = common::VecFromArray(imu_start.pos);
        const Eigen::Vector3d t_end = common::VecFromArray(imu_end.pos);
    
        // 计算整体旋转变化: delta_R = R_start^T * R_end
        IMURel.delta_R = R_start.transpose() * R_end;
    
        // 计算整体平移变化: delta_t = R_start^T (t_end - t_start)
        IMURel.delta_t = R_start.transpose() * (t_end - t_start);
    
        // 计算时间间隔
        IMURel.delta_time = imu_end.offset_time - imu_start.offset_time;

        IMURelativePose_.push_back(IMURel);
        // 此时 imu_total_delta 即为 T_{B_{k-1}B_k}
    }
